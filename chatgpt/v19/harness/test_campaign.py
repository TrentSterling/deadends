...
with sync_playwright() as pw:
 ...
 for mode in ['classic','remix']:
  for m in range(5):
   start=time.time();p.evaluate('(v)=>{DEAD_ENDS[v.mode===\'classic\'?\'startMap\':\'startRemix\'](v.map, v.mode===\'classic\'?31987:123456);DEAD_ENDS.qa19.setAudio(false);DEAD_ENDS.setInvincible(true);DEAD_ENDS.setAuto(true);DEAD_ENDS.qa19.setSandbox(true)}',{'mode':mode,'map':m})
   trace=[]
   ...
   row={'mode':mode,'map':m,'seed':31987 if mode=='classic' else 123456,'invulnerable':True,'pass':r['phase']=='win','secondsWall':round(time.time()-start,2),'final':r,'trace':trace};out['runs'].append(row)
   (W/'qa_campaign.json').write_text(json.dumps(out,indent=2));print(mode,m,'PASS' if row['pass'] else 'FAIL',round(r['progress'],3),round(r['t'],2),r['mission'],flush=True)
   if not row['pass']:
    p.evaluate('DEAD_ENDS.qa19.render()');p.screenshot(path=str(W/f'stall_{mode}_{m}.png'))
 out['pass']=all(r['pass'] for r in out['runs']) and not out['errors'];(W/'qa_campaign.json').write_text(json.dumps(out,indent=2));b.close()
